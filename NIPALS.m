...
nMaxOuter = 10000; %max number of itertion to convergence
a=3; % 0 <= a <= j_max
for j = 1 : a
    % choose The column of X has The largest square of sum as T.
    % choose The column of Y has The largest square of sum as U.
    [dummy,Tnum] =  max(diag(Ej'*Ej));
    [dummy,Unum] =  max(diag(Fj'*Fj));
    tj_prec = Ej(:,Tnum); %any column of X
    uj = Fj(:,Unum);      %any column of Y
...
Y_hat=X*B2;
%fo each row set the higer value to 1 and the others to 0 according to 6.4
for r=1:n
    max_row=max(Y_hat(r,:));
    for c=1:3
